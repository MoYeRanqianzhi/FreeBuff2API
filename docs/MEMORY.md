...
- 2026-04-17: **绑定式 donor key（v0.10.0）** —— 众筹登录成功后自动发放一把 `fb_donor_<hex>` 格式的 API key，作为对贡献者的反哺。该 key 写入 `auths/<label>.json` 的 `donorKey` 字段，与其上游账号严格 1:1；持有者用它做客户端 Bearer 调 `/v1/*` 时，proxy 会 pin 到对应上游账号：**账号限流 → 429、熔断 → 503、全部不跨账号兜底**。KeyPool 新增 `donors []string` 平行数组 + `ResolveDonorKey/SetDonorKey/IsBroken`；authGuard 识别 `fb_donor_` 前缀并填充 `ctxKeyPinnedKeyIdx/Upstream`；proxy 单独 `servePinned` 分支。admin 面板每行增 donor 列（生成/重置/复制/清除/自定义），`POST/DELETE /admin/api/keys/{label}/donor` 直接改写凭证 JSON。设计目的：防止贡献者恶意使用 key 清空整个号池
