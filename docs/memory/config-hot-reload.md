...
./freebuff2api -config ""               # 纯 env 兼容模式（不读配置文件）
```
...
旧版部署若使用 env 变量，升级后仍然有效：`applyEnvOverrides` 在 YAML 解析后执行，env 值覆盖 YAML。支持的 env：

| Env | 覆盖字段 |
|---|---|
| `LISTEN_ADDR` | `server.listen` |
| `API_KEY` | `server.api_key` |
| `FREEBUFF_BASE_URL` | `upstream.base_url` |
| `COST_MODE` | `upstream.cost_mode` |
| `DEFAULT_MODEL` | `upstream.default_model` |
| `FREEBUFF_API_KEY` / `FREEBUFF_API_KEYS` | 追加到 `auth.api_keys`（逗号/分号/换行分隔） |
| `AUTHS_DIR` | `auth.dir` |
| `AUTHS_WATCH_INTERVAL` | `auth.watch_interval` |
| `LOG_LEVEL` | `logging.level` |

注意 env 覆盖只在启动时与 reload 时求值——更改 env 需要重启或触发文件变化。建议新部署全权用 YAML。
...
- `TestLoadConfigMissingFileUsesEnv` — 文件缺失 → env 兜底
- `TestLoadConfigEnvOverridesYAML` — env 覆盖 YAML
